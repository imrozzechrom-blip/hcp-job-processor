...
export async function handleJobs(
  ...
  company: ICompany,
  deps: {
    CallRailCustomer: CallRailCustomerRepo;
    HcpJob: HcpJobRepo;
    HistoricalAverage: HistoricalAverageRepo;
    calculateLostRevenueInJob: Function;
    findBestCallrailCustomer: Function;
    getLatLonFromGeocode: Function;
  }
) {
  ...
  if (eventType === "job.deleted") {
    ...
    const existingHcpJob = await findExistingCombinedJob(jobId, companyId);
    ...
    if (existingHcpJob) {
      ...
      if (callCustomerId) {
        ...
        await CallRailCustomer.findOneAndUpdate(
          {
            _id: callCustomerId,
            tags: {
              $nin: [
  ...
  const existingHcpJob = await HcpJob.findOne({
  ...
  if (!existingHcpJob && zip) {
    ...
    mapData = await getLatLonFromGeocode(address);
  ...
  const phoneOptions = [
    normalizePhone(job.customer.mobile_number || ""),
    normalizePhone(job.customer.home_number || ""),
  ].filter(Boolean);
  ...
  for (const phone of phoneOptions) {
    ...
    const callsForPhone = await CallRailCustomer.find({
  ...
  const matchResult = await findBestCallrailCustomer(
    uniqueCallrailCustomers,
    normalizeDate(job.created_at),
    async (callrailId: string) => {
      const existingJob = await HcpJob.findOne({
  ...
  let historicalDoc = (await HistoricalAverage.findOne({
    companyId,
    date: targetDate,
  }).lean()) as IHistoricalAverage | null;

  if (!historicalDoc) {
    historicalDoc = (await HistoricalAverage.findOne({ companyId })
      .sort({ date: -1 })
      .lean()) as IHistoricalAverage | null;
  }
  ...
  if (callrailCustomer) {
    ...
    const linkedEstimate = await HcpEstimate.findOne({
  ...
  const createOrCombineJob = async (jobData: any, status: string) => {
    if (callrailCustomer) {
      ...
      const newJob = new HcpJob({ ...jobData, jobStatus: status });
      await newJob.save();
      console.log(
      ...
      const mostRecentLinkedJob = await findMostRecentLinkedCallJob(
        uniqueCallrailCustomers,
        jobTimestamp,
        companyId,
      );
  ...
  if (eventType === "job.created") {
    const existingHcpJob = await findExistingCombinedJob(jobId, companyId);
    await calculateLostRevenueInJob(jobId, job, jobStatus, callrailCustomer, company);
    if (existingHcpJob) {
      const callCustomerId = existingHcpJob.toObject().callrailCustomerId;
      if (callCustomerId) {
        // Append "Later Qualified" to tags if none of the excluded tags are present (Because if job is exist that means that call is qualified - so updating the database)
