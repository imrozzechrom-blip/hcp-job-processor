...
exports.findExistingCombinedJob = findExistingCombinedJob;
exports.findMostRecentLinkedCallJob = findMostRecentLinkedCallJob;
exports.updateParentJobFromPrimary = updateParentJobFromPrimary;
exports.findPrimaryJobComponent = findPrimaryJobComponent;
async function handleJobs(job, eventType, // Fixed: Was invalid "string | ,"
jobId, company, deps) {
    ...
    if (eventType === "job.deleted") {
        ...
        if (existingHcpJob) {
            ...
            if (existingHcpJob.jobComponents && existingHcpJob.jobComponents.length > 0) {
                const componentIndex = existingHcpJob.jobComponents.findIndex((comp) => comp.hcpId === jobId);
                if (componentIndex !== -1) {
            ...
            console.warn(`⚠️ Job ${jobId} not found in DB during 'job.deleted' event. Cannot mark as canceled.`);
        }
    ...
    if (isNaN(dateRange.getTime()) || isNaN(extendedEndDate.getTime())) {
        console.error("❌ Invalid dateRange or extendedEndDate calculated:", dateRange || extendedEndDate);
    ...
    for (const phone of phoneOptions) {
        if (!phone)
            continue;
        console.log(`🔍 Searching for calls with phone ${phone} from ${dateRange.toISOString()} to ${jobTimestamp.toISOString()}`);
    ...
    const uniqueCallrailCustomers = allCallrailCustomers.filter((call, index, self) => index === self.findIndex(c => c._id.toString() === call._id.toString()));
    console.log(`📋 Total unique calls found: ${uniqueCallrailCustomers.length} (from ${Object.keys(phoneCallCounts).length} phone numbers)`);
    ...
    const matchResult = await deps.findBestCallrailCustomer(uniqueCallrailCustomers, deps.normalizeDate(job.created_at), async (callrailId) => {
    ...
    if (callrailCustomer) {
        ...
        const timeDiffHours = Math.abs(jobTimestamp.getTime() - callCreatedAt.getTime()) / (1000 * 60 * 60);
        ...
        const enhancedMatchDetails = {
            ...
            matchedPhone: phoneOptions.find(phone => callrailCustomer.customer_phone_number === phone) || "Unknown",
            ...
            matchQuality: timeDiffDays <= 1
                ...
                : timeDiffDays <= 7
                    ...
                    : timeDiffDays <= 31
                        ? "Acceptable"
                        : "Poor",
            matchConfidence: timeDiffDays <= 1
                ? "Very High"
                : timeDiffDays <= 3
                    ? "High"
                    : timeDiffDays <= 7
        ...
        console.log(`   📊 Match Quality: ${enhancedMatchDetails.matchQuality} (${enhancedMatchDetails.matchConfidence} confidence)`);
        console.log(`   ⏰ Time Difference: ${enhancedMatchDetails.timeDifference.days} days (${enhancedMatchDetails.timeDifference.hours} hours)`);
        ...
        console.log(`   🔗 Estimate Linkage: ${enhancedMatchDetails.estimateLinkage.estimateJobRelationship}`);
        ...
        const noMatchDetails = {
            ...
            potentialIssues: uniqueCallrailCustomers.length === 0
                ? "No calls found in search window"
                : "All calls already linked to other jobs",
            recommendations: uniqueCallrailCustomers.length === 0
        ...
        console.log(`   🔍 Search Window: ${noMatchDetails.searchWindow.start} to ${noMatchDetails.searchWindow.end}`);
    ...
    const combineWithExistingJob = async (jobData, existingJob) => {
        ...
        const existingComponentIndex = existingJob.jobComponents.findIndex((comp) => comp.hcpId === jobId);
        ...
        existingJob.revenue = existingJob.jobComponents.reduce((total, comp) => total + (comp.revenue || 0), 0);
    ...
    const createOrCombineJob = async (jobData, status) => {
        if (callrailCustomer) {
            ...
            await newJob.save();
            console.log(`✅ Created new job ${jobId} with status '${status}' and linked to call ${callrailCustomer._id}`);
        }
        else {
            // If no direct call match, try to find a related job to combine with.
            const mostRecentLinkedJob = await deps.findMostRecentLinkedCallJob(uniqueCallrailCustomers, jobTimestamp, companyId);
            if (mostRecentLinkedJob) {
                ...
                const newJob = deps.HcpJob.new({ ...jobData, jobStatus: status }); // Fixed: Use deps.HcpJob.new
                await newJob.save();
                console.log(`✅ Created new standalone job ${jobId} with status '${status}' (no call link).`);
            }
        }
    };
    ...
    if (eventType === "job.created") {
        ...
        if (existingHcpJob) {
            ...
            if (existingHcpJob.jobComponents && existingHcpJob.jobComponents.length > 0) {
                ...
                existingHcpJob.revenue = existingHcpJob.jobComponents.reduce((total, comp) => total + (comp.revenue || 0), 0);
                // Update parent record from the primary component
                updateParentJobFromPrimary(existingHcpJob);
                await existingHcpJob.save();
            }
                ...
                await deps.HcpJob.updateOne({ hcpId: jobId, companyId: company._id }, {
                    $set: {
                        jobStatus: "created", // Set correct status
                        revenue: job.total_amount,
                        customer: jobDoc.customer,
        ...
        if (existingHcpJob) {
            ...
            if (!existingHcpJob.callrailCustomerId && callrailCustomer?._id) {
                console.log(`🔗 Attaching newly found call ${callrailCustomer._id} to existing job ${jobId}.`);
                if (callrailCustomer?._id) {
                    // Append "Later Qualified" to tags if none of the excluded tags are present (Because if job is exist that means that call is qualified - so updating the database)
                    await deps.CallRailCustomer.findOneAndUpdate({
                        _id: callrailCustomer._id,
    ...
    else if (eventType === "job.canceled" ||
        ...
        if (existingHcpJob) {
            // If the existing job doesn't have a call linked, but we found one now, attach it.
            if (!existingHcpJob.callrailCustomerId && callrailCustomer?._id) {
                console.log(`🔗 Attaching newly found call ${callrailCustomer._id} to existing job ${jobId} during cancellation.`);
                if (callrailCustomer?._id) {
                    // Append "Later Qualified" to tags if none of the excluded tags are present (Because if job is exist that means that call is qualified - so updating the database)
                    await deps.CallRailCustomer.findOneAndUpdate({
                        _id: callrailCustomer._id,
        ...
        if (existingHcpJob) {
            // If the existing job doesn't have a call linked, but we found one now, attach it.
            if (!existingHcpJob.callrailCustomerId && callrailCustomer?._id) {
                console.log(`🔗 Attaching newly found call ${callrailCustomer._id} to existing job ${jobId} during completion.`);
                if (callrailCustomer?._id) {
                    // Append "Later Qualified" to tags if none of the excluded tags are present (Because if job is exist that means that call is qualified - so updating the database)
                    await deps.CallRailCustomer.findOneAndUpdate({
                        _id: callrailCustomer._id,
...
async function findExistingCombinedJob(HcpJob, jobId, companyId) {
...
const qualifiedTags = ["qualified"];
const normalizeTags = (tag) => tag.toLowerCase().trim();
async function findMostRecentLinkedCallJob(uniqueCallrailCustomers, jobTimestamp, companyId, deps // Note: This function needs deps for HcpJob, so pass it in when calling
) {
    if (!uniqueCallrailCustomers || uniqueCallrailCustomers.length === 0) {
        return null;
    }
    // Separate calls into qualified and non-qualified
    const qualifiedCalls = uniqueCallrailCustomers.filter(call => call.tags?.some((tag) => qualifiedTags.includes(normalizeTags(tag))));
    const nonQualifiedCalls = uniqueCallrailCustomers.filter(call => !qualifiedCalls.some(qc => qc._id.toString() === call._id.toString()));
    // Sort both lists by most recent first
    const sortCallsByDate = (calls) => calls.sort((a, b) => {
    ...
    for (const call of sortedQualifiedCalls) {
        ...
        if (existingJob) {
            const daysDifference = (jobTimestamp.getTime() - new Date(existingJob.created_at).getTime()) / (1000 * 3600 * 24);
            if (daysDifference >= 0 && daysDifference <= 30) {
                console.log(`✅ Found QUALIFIED linked call ${call._id} with job ${existingJob.hcpId}. Combining.`);
                return existingJob;
            }
        }
    }
    ...
    for (const call of sortedNonQualifiedCalls) {
        ...
        if (existingJob) {
            const daysDifference = (jobTimestamp.getTime() - new Date(existingJob.created_at).getTime()) / (1000 * 3600 * 24);
            if (daysDifference >= 0 && daysDifference <= 30) {
                console.log(`✅ Found NON-QUALIFIED linked call ${call._id} with job ${existingJob.hcpId}. Combining.`);
                return existingJob;
            }
        }
    }
...
function updateParentJobFromPrimary(jobRecord) {
    ...
    if (!primaryComponent?._doc)
        return;
    const newStatus = primaryComponent._doc.jobStatus === "updated"
        ? "created"
...
function findPrimaryJobComponent(components) {
